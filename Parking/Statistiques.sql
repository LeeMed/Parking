...
from (select NOM_PARKING, COUNT(*) AS "PLACE_DISPO_BAR" from STATIONNEMENT S inner join PARKING P on P.ID_PARKING = S.ID_PARKING
     where (S.DATE_STATIONNEMENT_E <= '2022-11-23 16:14:0' AND (S.DATE_STATIONNEMENT_S >= '2022-11-23 16:14:0' OR S.DATE_STATIONNEMENT_S is null))
     group by NOM_PARKING) AS T;

select NUMERO_IMMATRICULATION, NOM_PARKING, AVG(DUREE_STAT) as "durée moyenne voit/par"
from(select NUMERO_IMMATRICULATION, NOM_PARKING,TIMESTAMPDIFF(HOUR,S.DATE_STATIONNEMENT_S,S.DATE_STATIONNEMENT_E) AS "DUREE_STAT"
...
from (select NUMERO_IMMATRICULATION,NOM_PARKING, SUM(P.TARIF * TIMESTAMPDIFF(HOUR,S.DATE_STATIONNEMENT_S,S.DATE_STATIONNEMENT_E)) AS "COUT_STAT" 
